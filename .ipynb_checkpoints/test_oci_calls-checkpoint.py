...
url = "https://modeldeployment.us-ashburn-1.oci.customer-oci.com/ocid1.datasciencemodeldeployment.oc1.iad.amaaaaaa3mg5kzaaapwt6jn5xov2v4ye35jbxdqcc332x6e2vfo7bzice3yq/predict"

# inspired from OCI Python SDK GitHub
def make_prediction(auth, input_body):
    print('')
    ...
    try:
        ...
        prediction = requests.post(url, json=json.dumps(input_body), auth=auth)

        if prediction.status_code == 200:
            print('Model predictions:')

            # extract vector
            preds = prediction.json()['prediction']
            print(preds)
            print('')
        else:
            print('')
            print("Making predictions failed !!!")
            print('Status code:', prediction.status_code)
...
def prepare_input_data(start, end):
    ...
    input_data = dati_test[start:end].values
